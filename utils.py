...
def change_ct_domain(input_path, sigma=1.0):
    input_image     = sitk.ReadImage(input_path)
    gaussian_filter = sitk.SmoothingRecursiveGaussianImageFilter()
    gaussian_filter.SetSigma(sigma)
    input_image = gaussian_filter.Execute(input_image)
    input_array = sitk.GetArrayFromImage(input_image)

    # Normalize the intensity using the cosine function
    max_value    = np.max(input_array)
    min_value    = np.min(input_array)
    alpha        = np.pi / (max_value - min_value)
    beta         = -np.pi * min_value / (max_value - min_value)
    output_image = (np.cos(alpha * input_array + beta) + 1) / 2
