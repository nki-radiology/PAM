...
class Preprocessing(object):
    ...
    def get_difference_days(self, ct1_date, ct2_date):
        ...
        #input_date_list.sort(key=lambda date: datetime.strptime(date, "%Y-%m"))
    ...
    def get_difference_between_scans(self, yads):
        ...
        for i in range(len(yads)):
            ...
            directories_date.sort(key = lambda x: datetime.strptime(x['date_directory'], '%Y-%m-%d'))
            # Get the values
            max_len = len(directories_date) - 1
            for j in range (max_len):
                idx = max_len - j
                idx_prev = max_len - j - 1
                difference_in_days_between_scans = self.get_difference_days(directories_date[idx_prev]["date_directory"], directories_date[idx]["date_directory"])
                pairs.append([ yads.iloc[i]['AnonymizedName'] , yads.iloc[i]['PatientID'], yads.iloc[i]['YADSId'], yads.iloc[i]['AnonymizedPatientID'], 
                    yads.iloc[i]['Included'], directories_date[idx_prev]["date_directory"], 
                    yads.iloc[i]['PathRawDicom'] + directories_date[idx_prev]["date_directory"] + directories_date[idx_prev]["remaining_name_directory"], 
                    directories_date[idx]["date_directory"],
                    yads.iloc[i]['PathRawDicom'] + directories_date[idx]["date_directory"] + directories_date[idx]["remaining_name_directory"],
